...
1. **`src/video_c.h`**  
   `FrameCallback` の typedef 直上のコメントブロックに、**日本語で**次を短く書く（英語のみの注意書きは不可）:
...
- `video_c.h`: `FrameCallback` 直上に日本語でパニック禁止・ポインタ寿命・`to_owned()`（約 33〜35 行）。英語ブロックは削除済み。
