...
USB Web カメラの多くは、**高解像度 (1080p 以上)** や **高フレームレート (30 fps 以上)** で **MJPEG (Motion JPEG, V4L2_PIX_FMT_MJPEG)** を主要フォーマットとして公開する。USB 2.0 (480 Mbps) では非圧縮 `YUYV` の 1080p/30fps を流せないため、カメラ側が **MJPEG のみを公開する** ケースが多い。

現在の V4L2 バックエンドは **NV12 / YUY2 / I420 の 3 フォーマットしか認識しない** ため、

- `enumerate_device_formats` (`src/video_v4l2.c:54-65`) は MJPEG を `default: 0` で除外する
...
pipewire = []
v4l2 = []
mjpeg = ["v4l2"]   # 新規追加。v4l2 に依存
```

現在の動機:

1. パススルーで届く **破損 JPEG の取り扱い責務** を負える利用者だけが opt-in する設計が、CLAUDE.md「性能より堅牢性を優先」と整合する
...
1. `build.rs::build_linux_v4l2` に `env::var("CARGO_FEATURE_MJPEG").is_ok()` を見て `cc::Build::define("SHIGUREDO_VIDEO_DEVICE_MJPEG", None)` を呼ぶ分岐を追加する
2. `src/video_v4l2.c` の MJPEG 関連コード (`convert_v4l2_pixel_format` / `convert_video_pixel_format_to_v4l2` / `capture_thread` の MJPEG 分岐、`MJPEG_MAX_PAYLOAD_BYTES` 定数) を `#ifdef SHIGUREDO_VIDEO_DEVICE_MJPEG` で囲む
3. `src/video_c.h` の `VIDEO_PIXEL_FORMAT_MJPG` 定数と `FrameCallback` の MJPEG 仕様コメントは **feature ガードしない** (C ABI ヘッダとして常に提供。第三者再利用者が必要とする可能性)
...
1. CLAUDE.md 「**依存は最小限にすること**」と整合する (JPEG デコーダ依存を持ち込まない)
2. CLAUDE.md 「**性能より堅牢性を優先すること**」と整合する (JPEG パースのバグ・脆弱性を抱えない)
...
C コールバック `FrameCallback` (`src/video_c.h:38-47`) の **シグネチャは変更しない**。MJPEG の長さは `stride` 引数経由で Rust 側に伝達する。

`stride` 引数の意味が `pixel_format` に依存する状態は、C ABI を第三者再利用する利用者から見て型詐欺になり得る。**`video_c.h` のコメントに明示的契約として記述** する:
...
- 既存実機テスト `test_capture_frames` (`tests/test_capture.rs:75`) の `assert!(frame.stride > 0)` も既定 (NV12/YUYV) で動作するため修正不要

### macOS でのエラー型統一

feature 有効時に `VideoCaptureConfig::pixel_format = Some(PixelFormat::Mjpeg)` を macOS / Windows に渡したときのエラー型を `Error::UnsupportedPixelFormat(Mjpeg)` に統一する:

- Windows: `pixel_format_to_guid(Mjpeg) = None` → 既存経路で `Error::UnsupportedPixelFormat(Mjpeg)` (`capture_windows.rs:306-310`)
- macOS: 現状経路では `convert_video_pixel_format_to_cv` の `default: 0` (`video_c.m:131-143`) → C 側 NULL 返却 → `Error::SessionCreateFailed`。これを揃えるため、`src/capture.rs::VideoCapture::new` の入口に macOS 用早期エラー分岐を追加する

```rust
#[cfg(all(feature = "mjpeg", target_os = "macos"))]
if matches!(config.pixel_format, Some(PixelFormat::Mjpeg)) {
    return Err(Error::UnsupportedPixelFormat(PixelFormat::Mjpeg));
}
```

`capture.rs` は Linux / macOS 共通モジュールであり、macOS 固有コードを置く責務分離は弱まる。`capture_macos.rs` への分割は本 issue のスコープ外 (別 issue)。
...
- `PixelFormat::Mjpeg` バリアントとその FourCC 値の expose
- macOS で `Some(PixelFormat::Mjpeg)` を渡したときのエラー型を `UnsupportedPixelFormat(Mjpeg)` に統一
- Windows 側 `pixel_format_to_guid` / `process_sample` の `match` 網羅維持 (バリアント追加に伴う機械的な付随変更)

**対象外 (別 issue)**:

- PipeWire の MJPEG 対応 (`SPA_VIDEO_FORMAT_ENCODED` 経由で構造が異なる)
- Windows Media Foundation での MJPEG 対応 (列挙とキャプチャをセットで別 issue)。本 issue では `guid_to_pixel_format` への `MFVideoFormat_MJPG` マップ追加は行わない (列挙だけ Linux と揃えても Windows ではキャプチャ不能のため、`closed/0016` 流の不整合許容よりも「Linux 専用機能」と明確に切る方が API 整合性が高い)
...
- `Cargo.toml:45-48` — features は `default = ["v4l2"]`, `pipewire = []`, `v4l2 = []` の 3 件
- `build.rs:57-67` — `build_linux_v4l2` は `video_v4l2.c` を `cc::Build` でコンパイル。feature 連動の `define` なし
- `src/video_c.h:13-15` — `VIDEO_PIXEL_FORMAT_*` 定数は 3 種類のみ
- `src/types.rs:23-32` — `PixelFormat` enum は 4 バリアント (`#[non_exhaustive]` 非付与)
- `src/types.rs:348-357` — Windows: `pixel_format_to_guid` は `match`。バリアント追加時は分岐追加が必須
- `src/video_v4l2.c:54-78` — V4L2 ↔ FourCC 変換テーブル。MJPEG 分岐なし
- `src/video_v4l2.c:103-106` — `enumerate_device_formats` は `convert_v4l2_pixel_format` が 0 を返すフォーマットをスキップ
...
- `src/capture.rs:25-31` — `VideoCapture::new` の入口で `Unknown(_)` は早期エラー
- `src/capture.rs:113-148, 150-247` — Rust 側 `frame_callback` と `*_plane_sizes` ヘルパ群
- `src/capture_windows.rs:515-601` — `process_sample` の `match pixel_format`。各分岐は `buffer.Unlock()` を呼んでから return / 処理
- `src/video_c.m:131-143` — macOS `convert_video_pixel_format_to_cv` は MJPEG を `default: 0` で除外
...
`build_linux_v4l2` に feature 連動の `define` を追加:

```rust
fn build_linux_v4l2(src_dir: &Path) {
    println!("cargo::rerun-if-changed=src/video_v4l2.c");

    let has_mjpeg = env::var("CARGO_FEATURE_MJPEG").is_ok();
...
`capture_thread` (`src/video_v4l2.c:574-612`) の MJPEG 分岐は、既存の `else if (session->pixel_format == V4L2_PIX_FMT_YUYV)` (603 行) の後ろに `#ifdef SHIGUREDO_VIDEO_DEVICE_MJPEG` ガード付きで `else if (session->pixel_format == V4L2_PIX_FMT_MJPEG)` として追加:

- `bytesused == 0` は既存ガード (567 行) でスキップ済み
- `available = min(bytesused, mmap_len)` も既存どおり
- **絶対上限ガード**: `if (available > MJPEG_MAX_PAYLOAD_BYTES) goto requeue;` を入れる。CLAUDE.md「ログはできるだけださない」方針に従い、上限超過のログ出力は行わない (サイレントスキップ)
...
`PixelFormat::Mjpeg` バリアントと関連を `#[cfg(feature = "mjpeg")]` でガード。`to_raw` / `from_raw` 自身が既に `cfg(any(macos, linux))` で囲まれているので、内側の match 分岐には `#[cfg(feature = "mjpeg")]` のみを付与する (cfg 重複を避ける):

```rust
#[cfg(all(feature = "mjpeg", any(target_os = "macos", target_os = "linux")))]
...
pub enum PixelFormat {
    ...
    /// `Mjpeg.to_raw()` は Linux / macOS でのみ利用可能 (Windows では関数自体が cfg ガードされる。
    /// `closed/0014` の API 対称性方針を継承)。
...
- `from_raw` (`src/types.rs:36-44`) の match に `#[cfg(feature = "mjpeg")] VIDEO_PIXEL_FORMAT_MJPG => PixelFormat::Mjpeg` を追加
- `to_raw` (`src/types.rs:49-55`) の match に `#[cfg(feature = "mjpeg")] PixelFormat::Mjpeg => VIDEO_PIXEL_FORMAT_MJPG` を追加
- `name` (`src/types.rs:58-66`) の match に `#[cfg(feature = "mjpeg")] PixelFormat::Mjpeg => "MJPEG"` を追加
- Windows 側 `pixel_format_to_guid` (`src/types.rs:348-357`) に **`#[cfg(feature = "mjpeg")] PixelFormat::Mjpeg => None`** を追加 (網羅性維持、必須)
- Windows 側 `guid_to_pixel_format` は **変更しない** (本 issue では Windows MJPEG 列挙対応はスコープ外)

`VideoFrame` / `VideoFrameOwned` の rustdoc 更新 (`src/types.rs:200-280` 付近):
...
`VideoCaptureConfig::pixel_format` (`src/types.rs:184` 付近) の rustdoc に、`mjpeg` feature 有効時に macOS / Windows で `Some(PixelFormat::Mjpeg)` を渡すと `UnsupportedPixelFormat(Mjpeg)` で失敗する旨を明記。

### 6. `src/lib.rs`

クレートドキュメント (`src/lib.rs:12`) に独立した 2 文として追記:
...
すべての MJPEG 関連コードを `#[cfg(feature = "mjpeg")]` でガードする。

macOS 用早期エラー化を `VideoCapture::new` の冒頭 (関数の最初の文として、既存の `requested_pixel_format` 計算 (25-31 行) より前) に追加:
...
`mjpeg_payload_bytes(payload_size: i32) -> Option<usize>` ヘルパを `nv12_plane_sizes` / `i420_plane_sizes` / `yuy2_packed_frame_bytes` (113-148 行) と同じ位置に `#[cfg(feature = "mjpeg")]` ガード付きで追加。引数型 `i32` は `frame_callback` の `stride: i32` を流用する整合性のため。関数定義コメントで「C ABI の `stride` 引数 (i32) を長さスロットに流用する経路。呼び出し側は `mjpeg_payload_bytes(stride)` の形になる」と明示する。`payload_size <= 0` を拒否し `Some(payload_size as usize)` を返す。

`match pf` (174 行) に `#[cfg(feature = "mjpeg")]` ガード付きで新分岐:

```rust
#[cfg(feature = "mjpeg")]
PixelFormat::Mjpeg => {
    let Some(data_size) = mjpeg_payload_bytes(stride) else { return };
...
### 8. `src/capture_windows.rs`

`process_sample` (`src/capture_windows.rs:515-601`) の `match pixel_format` に `#[cfg(feature = "mjpeg")]` ガード付き分岐を追加。既存アーム順 `Nv12 / I420 / Yuy2 / Unknown(_)` に対し、**`Yuy2` アームの直後、`Unknown(_)` アームの直前** に挿入。他アームと同じく `buffer.Unlock()` を呼んでから return:
...
このリポジトリには `pbt/` ディレクトリが存在しないため、本 issue で PBT 基盤を新規構築しない。すべての MJPEG テストは `#[cfg(feature = "mjpeg")]` ガード付きで追加する。
...
5. `VideoFrame::to_owned` および `VideoFrameOwned::as_frame` のラウンドトリップ MJPEG ケース。`data` には JPEG マジック (`0xFF, 0xD8, ...`) を含むダミーバイト列を入れる (人間可読のダミーデータの意図に過ぎず、`data[0..2] == [0xFF, 0xD8]` をアサート対象としない)

`src/capture.rs::#[cfg(test)] mod tests` (250-294 行) へのヘルパ単体テスト追加 (計 2 件、いずれも `#[cfg(feature = "mjpeg")]` ガード)。既存命名規約 (`nv12_rejects_non_positive_dimensions` / `nv12_small_known_sizes` 風) に揃える:
...
- [ ] `Cargo.toml`: `mjpeg = ["v4l2"]` feature 追加。`default` 変更なし
- [ ] `build.rs`: `build_linux_v4l2` で `CARGO_FEATURE_MJPEG` を見て `cc::Build::define("SHIGUREDO_VIDEO_DEVICE_MJPEG", None)` を呼ぶ
- [ ] `src/video_c.h`: `VIDEO_PIXEL_FORMAT_MJPG` 定数 (feature ガードなし)、`FrameCallback` の MJPEG 仕様 (stride 単位が pixel_format に依存する旨を含む明示的契約) コメントを追加
...
- [ ] `src/types.rs`: `#[cfg(feature = "mjpeg")]` ガード付きで `VIDEO_PIXEL_FORMAT_MJPG` 定数、`PixelFormat::Mjpeg` バリアント、`from_raw` / `to_raw` / `name` の match 分岐、Windows `pixel_format_to_guid` の `Mjpeg => None` を追加。`VideoFrame` / `VideoFrameOwned` / `VideoCaptureConfig` の rustdoc に MJPEG 時の解釈、`V4L2_BUF_FLAG_ERROR` 警告、macOS / Windows でのエラー型統一を明記
- [ ] `src/lib.rs`: クレートドキュメントに `mjpeg` feature 有効化時の MJPEG 既知扱いと `closed/0017` 方針維持を独立 2 文で追記
- [ ] `src/capture.rs`: `#[cfg(feature = "mjpeg")]` ガード付きで `mjpeg_payload_bytes` ヘルパ (引数名 `payload_size: i32`、用途コメント付き)、`frame_callback` の MJPEG 分岐、`VideoCapture::new` の macOS 用早期エラー分岐、`#[cfg(test)] mod tests` への単体テスト 2 件を追加
- [ ] `src/capture_windows.rs`: `process_sample` の `match` に `#[cfg(feature = "mjpeg")] PixelFormat::Mjpeg => { let _ = buffer.Unlock(); return; }` を追加
...
- [ ] ローカル (Linux 環境) で以下の組み合わせで `cargo build` / `cargo clippy` / `cargo test` がすべて通る。`tests/test_capture.rs` は `#[ignore]` 付きで通常実行されないため、対象は `tests/test_types.rs` の MJPEG 関連 5 件と `src/capture.rs` 内 `mod tests` の MJPEG 関連 2 件:
...
- **`mjpeg` feature 有効時 (macOS)**: `Some(PixelFormat::Mjpeg)` 指定時のみ `Error::UnsupportedPixelFormat(Mjpeg)` を返す (早期エラー分岐)。`formats()` には MJPEG は出現しない (`video_c.m::convert_pixel_format` が MJPEG を `default: 0` で除外)
