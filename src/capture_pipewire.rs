...
pub struct PipewireVideoCapture {
    session: Option<NonNull<ffi::pipewire::VideoSession>>,
    context: Option<Box<CaptureContext>>,
    config: VideoCaptureConfig,
}

impl VideoCapture for PipewireVideoCapture {
    fn start(&mut self) -> Result<()> {
        ...
        let ret = unsafe {
            ffi::pipewire::video_pipewire_session_start(
                session.as_ptr(),
                Some(frame_callback),
                context_ptr,
            )
    ...
    fn stop(&mut self) {
            ...
            && context.running.load(Ordering::Acquire)
        {
            if let Some(session) = self.session {
                unsafe { ffi::pipewire::video_pipewire_session_stop(session.as_ptr()) };
            }
...
impl PipewireVideoCapture {
        ...
        F: Fn(VideoFrame<'_>) + Send + 'static,
    {
        ...
        let session = unsafe {
            ffi::pipewire::video_pipewire_session_create(
                device_id_ptr,
                config.width,
                config.height,
                config.fps,
...
impl Drop for PipewireVideoCapture {
    fn drop(&mut self) {
        self.stop();
        if let Some(session) = self.session.take() {
            unsafe { ffi::pipewire::video_pipewire_session_destroy(session.as_ptr()) };
        }
    }
}

unsafe impl Send for PipewireVideoCapture {}
...
extern "C" fn frame_callback(
    ...
    // context の生存期間は PipewireVideoCapture によって保証される
