...
pub struct AvfVideoCapture {
    session: Option<NonNull<ffi::avf::VideoSession>>,
    context: Option<Box<CaptureContext>>,
    config: VideoCaptureConfig,
}

impl VideoCapture for AvfVideoCapture {
    fn start(&mut self) -> Result<()> {
        ...
        let ret = unsafe {
            ffi::avf::video_avf_session_start(session.as_ptr(), Some(frame_callback), context_ptr)
        };
    ...
    fn stop(&mut self) {
            ...
            && context.running.load(Ordering::Acquire)
        {
            if let Some(session) = self.session {
                unsafe { ffi::avf::video_avf_session_stop(session.as_ptr()) };
            }
...
impl AvfVideoCapture {
        ...
        F: Fn(VideoFrame<'_>) + Send + 'static,
    {
        ...
        let session = unsafe {
            ffi::avf::video_avf_session_create(
                device_id_ptr,
                config.width,
                config.height,
                config.fps,
...
impl Drop for AvfVideoCapture {
    fn drop(&mut self) {
        self.stop();
        if let Some(session) = self.session.take() {
            unsafe { ffi::avf::video_avf_session_destroy(session.as_ptr()) };
        }
    }
}

unsafe impl Send for AvfVideoCapture {}
...
extern "C" fn frame_callback(
    ...
    // context の生存期間は AvfVideoCapture によって保証される
