...
//!
//! `mjpeg` feature を有効化すると、Linux V4L2 バックエンドで `V4L2_PIX_FMT_MJPEG` が `PixelFormat::Mjpeg` として
//! 既知扱いになり、MJPEG カメラから JPEG ペイロードを直接受け取れます。
//! それ以外の未知 FourCC を扱う `closed/0017` の方針 (`PixelFormat::Unknown(_)` でコールバックに渡らない) は維持されます。
//!
