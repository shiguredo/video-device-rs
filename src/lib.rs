...
//!
//! フレームコールバックは **panic してはなりません**。panic した場合の挙動はバックエンドにより異なります。
//! macOS / Linux (FFI) ではプロセスが abort しうる一方、Windows (Media Foundation) ではキャプチャスレッドが
//! 終了し、その後の再 [`VideoCapture::start`] が [`Error::CaptureFaulted`] になります。
//! ライブラリはユーザコールバックの panic を `catch_unwind` では吸収しません。
//!
